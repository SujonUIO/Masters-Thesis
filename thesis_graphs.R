...
ggsave("Info_module.jpeg", plot = Info_module, width = 10, height = 6, dpi = 600,units = "in")
...
combined_plot <- wrap_plots(plots, ncol = 3) & theme(legend.position = "none")

# Combine the plots with the legend, placing the legend at the bottom
final_plot <- combined_plot / legend + plot_layout(heights = c(10, 2))

# Display the final combined plot
print(final_plot)

# Save the plot
ggsave("SE_comparison.jpeg", plot = final_plot, width = 15, height = 12, dpi = 600, units = "in")
...
landscape_test_length_ggplot <- function(data, x_test, y_test, ll = 0, hl = 86, 
  ...
  plot_data <- data.frame(J_MST = J_MST, J_Other = J_Other, theta = theta)
  ...
  # Create the plot using ggplot2
  p <- ggplot(plot_data, aes(x = J_MST, y = J_Other, color = theta)) +
    geom_point(alpha = 0.6) +
    geom_abline(intercept = 0, slope = 1, color = "grey", linetype = "solid", size = 0.7) +  # Main diagonal line
...
ggsave("TestLength_comparison.jpeg", plot = final_plot, width = 15, height = 12, dpi = 600, units = "in")
...
# Combine the plots with the legend, placing the legend at the bottom
final_bias_plot <- combined_bias_plot / bias_legend + plot_layout(heights = c(10, 2))

# Display the final combined plot
print(final_bias_plot)

# Save the plot 
ggsave("landscape_BIAS_comparison.jpeg", plot = final_bias_plot, width = 15, height = 12, dpi = 600, units = "in")
...
for (info_test_type in test_types_info) {
  ...
  p_info <- item_info_comparison_plot(data = combined_data_loaded, 
                                      ...
                                      x_axis_label = "MST:Mean Item Information", 
                                      y_axis_label = paste(info_test_type, ":Mean Item Information"),
...
# Combine the plots with the legend, placing the legend at the bottom
final_info_plot <- combined_info_plot / info_legend + plot_layout(heights = c(10, 2))

# Display the final combined plot
print(final_info_plot)

# Save the plot 
ggsave("landscape_Info_comparison.jpeg", plot = final_info_plot, width = 15, height = 12, dpi = 600, units = "in")
...
# Save the plot
ggsave("boxplot_test_length_comparison.jpeg", plot = boxplot_test_length, width = 10, height = 7, dpi = 600, units = "in")
...
# Save the combined plot 
ggsave("CATtogether.jpeg", plot = CATtogether, width = 12, height = 6, dpi = 600,units = "in")
